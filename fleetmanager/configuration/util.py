...
def update_single_vehicle(session: Session, vehicle: Vehicle, fields: set[str] | None = None):
    ...
    The xlsx import uses it, since the sheet only carries a subset of the columns.
    """
...
def parse_sheet_date(value):
    ...
    month first whenever the day is 12 or less. Real Excel dates and other text formats
    keep the pandas default.
    """
...
# the only columns the import writes; disabled, deleted, test_vehicle etc. are never touched by a sheet
...
def update_vehicle_metadata(
    ...
    # a blank cell keeps the current value, except for the columns in METADATA_CLEARABLE,
    # and columns the sheet does not carry are never touched
    count = 0
